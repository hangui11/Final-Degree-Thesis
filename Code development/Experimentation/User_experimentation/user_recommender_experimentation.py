...
if __name__ == "__main__":
    ...
    # Create the user-to-user based recommender
    userRecommender = user.UserToUser(ratings_train, movies, users_idy)
    userSim = []
    countSim = 0

    # Make the prediction of the user-to-user based recommender for each user in the validation set
    ...
    # Save the similarity of each user with the user-to-user based recommender in a csv file 
    userDF = pd.DataFrame(userSim, columns=['userId', 'userSim'])
    path = user_dir + '/userSim.csv'
